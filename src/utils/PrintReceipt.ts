...
// ── Flexible receipt data (accepts camelCase OR snake_case) ────────────────
interface ReceiptData {
    voucher_no?: string;
    voucherNo?: string;              // 👈 camelCase alias
    ...
    party_name?: string;
    partyName?: string;              // 👈 camelCase alias
    ...
    bankAccountName?: string;        // 👈 camelCase alias
    bank_account?: string | number;

    mode?: string;
    cheque_no?: string;
    chequeNo?: string | null;        // 👈 camelCase alias
    cheque_date?: string;
    chequeDate?: string | null;      // 👈 camelCase alias
    cheque_clear_date?: string;
    chequeClearDate?: string | null; // 👈 camelCase alias

    // cash-only
    cash_account_name?: string;
    cashAccountName?: string;        // 👈 camelCase alias
    cash_account?: string | number;

    // branch (from serializer, multi-branch aware)
    branch_name?: string;
    branchName?: string;             // 👈 camelCase alias
    branch_owner_name?: string;
    branch_ownerName?: string;       // 👈 camelCase alias
...
// ── Normalize row (camelCase → snake_case internal) ────────────────────────
function normalize(r: ReceiptData) {
    return {
        voucher_no:        pick(r.voucher_no, r.voucherNo),
        date:              pick(r.date),
        type:              pick(r.type),
        amount:            r.amount,
        narration:         pick(r.narration),

        party_name:        pick(r.party_name, r.partyName),
        op_account:        pick(r.op_account),

        party_address:     pick(r.party_address),
        party_city:        pick(r.party_city),
        party_state:       pick(r.party_state),
        party_country:     pick(r.party_country),
        party_pincode:     pick(r.party_pincode),
        party_phone:       pick(r.party_phone),
        party_mobile:      pick(r.party_mobile),
        party_email:       pick(r.party_email),
        party_gst_no:      pick(r.party_gst_no),
        party_pan_card:    pick(r.party_pan_card),

        bank_account_name: pick(r.bank_account_name, r.bankAccountName),
        bank_account:      pick(r.bank_account),

        mode:              pick(r.mode),
        cheque_no:         pick(r.cheque_no, r.chequeNo),
        cheque_date:       pick(r.cheque_date, r.chequeDate),
        cheque_clear_date: pick(r.cheque_clear_date, r.chequeClearDate),

        cash_account_name: pick(r.cash_account_name, r.cashAccountName),
        cash_account:      pick(r.cash_account),

        branch_name:       pick(r.branch_name, r.branchName),
        branch_owner_name: pick(r.branch_owner_name, r.branch_ownerName),
        branch_address:    pick(r.branch_address, r.branchAddress),
        branch_city:       pick(r.branch_city, r.branchCity),
        branch_state:      pick(r.branch_state, r.branchState),
        branch_country:    pick(r.branch_country, r.branchCountry),
        branch_pincode:    pick(r.branch_pincode, r.branchPincode),
        branch_phone:      pick(r.branch_phone, r.branchPhone),
        branch_email:      pick(r.branch_email, r.branchEmail),
    };
}

function buildBranchLines(r: ReturnType<typeof normalize>): string {
...
function buildPartyAddress(r: ReturnType<typeof normalize>): string {
...
export function generateReceiptHTML(receiptInput: ReceiptData, kind: "bank" | "cash"): string {
    const receipt = normalize(receiptInput);
