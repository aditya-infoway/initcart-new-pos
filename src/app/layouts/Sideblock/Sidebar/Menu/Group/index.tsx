...
export function Group({ data, showIcon = true }: { data: NavigationTree; showIcon?: boolean }) {
        ...
        <Collapse in={isOpened}>
          <div className="flex flex-col space-y-1.5">
            {data.childs.map((item) => {
              switch (item.type) {
                case "collapse":
                  return <CollapsibleItem key={item.path} data={item} showIcon={showIcon} />;
                case "item":
                  return <MenuItem key={item.path} data={item} showIcon={showIcon} />;
                default:
                  return null;
              }
            })}
