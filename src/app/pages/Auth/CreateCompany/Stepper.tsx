...
import { StepKey, useCreateCompanyFormContext } from "./CreateCompanyFormContext";
import { CreateCompanyStep } from "./index";

interface StepperProps {
  steps: CreateCompanyStep[];
  currentStep: number;
...
export function Stepper({ steps, currentStep, setCurrentStep }: StepperProps) {
  ...
  const { state } = useCreateCompanyFormContext();
  const stepStatus = state.stepStatus;
  const stepKeys = Object.keys(state.formData) as StepKey[];
