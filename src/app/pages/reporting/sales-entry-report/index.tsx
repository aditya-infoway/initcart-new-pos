...
import {
  ...
  BuildingOfficeIcon, CubeIcon, CurrencyRupeeIcon, EyeIcon,
  FunnelIcon, MagnifyingGlassIcon, PrinterIcon,
  ReceiptRefundIcon, ShoppingCartIcon, XMarkIcon,
...
// ✅ ADD: same auth context used by OutstandingReport page
...
// ── Items Detail Drawer ────────────────────────────────────────────────────
function ItemsDrawer({ isOpen, onClose, sale }: { isOpen: boolean; onClose: () => void; sale: SaleRecord | null }) {
  return (
        ...
        >
          ...
          <div className="hide-scrollbar grow space-y-4 overflow-y-auto px-5 py-5">
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
              {[
                { label: "Total Items", value: sale?.items.length ?? 0, bg: "bg-gradient-to-br from-blue-500 to-blue-700", Icon: CubeIcon },
                { label: "Grand Total", value: `₹${Number(sale?.grandTotal ?? 0).toFixed(2)}`, bg: "bg-gradient-to-br from-primary-500 to-primary-700", Icon: BanknotesIcon },
                { label: "Basic Amount", value: `₹${Number(sale?.totalBasic ?? 0).toFixed(2)}`, bg: "bg-gradient-to-br from-emerald-500 to-emerald-700", Icon: CurrencyRupeeIcon },
...
export default function SalesEntryReportPage() {
  ...
  // ✅ ADD: same role-gating as OutstandingReport page — superadmin AND
  // employee both get to pick any branch.
  ...
  const fetchSales = useCallback(async (branchId: string = selectedBranchId) => {
    setLoading(true);
    try {
      const params: Record<string, unknown> = { page: 1, page_size: 1000 };
      if (branchId) params.branch_id = branchId;

      const res = await Get("pos/salesentry-list/", params) as any;
  ...
  // calling fetchSales() before setSelectedBranchId's re-render lands).
  ...
  return (
    <Page title="Sales Entry Report">
      ...
      {/* Items Detail Drawer */}
      <ItemsDrawer isOpen={!!selectedSale} onClose={() => setSelectedSale(null)} sale={selectedSale} />
    </Page>
  );
}