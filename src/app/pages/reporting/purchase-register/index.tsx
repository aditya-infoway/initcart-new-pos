...
import {
  ...
  CubeIcon, CurrencyRupeeIcon, EyeIcon, FunnelIcon,
...
// ✅ ADD: same auth context used by OutstandingReport + SalesEntryReport
...
function mapApiPurchase(raw: any): PurchaseRecord {
  ...
  return {
    ...
    purchaseBillNo: String(raw.purchase_bill_no ?? raw.pur_bill_no ?? raw.p_bill_no ?? ""),
    date:           String(raw.date ?? ""),
    partyName:      String(raw.party_name ?? raw.vendor_name ?? raw.supplier_name ?? ""),
...
// ── Items Detail Drawer ────────────────────────────────────────────────────
function ItemsDrawer({ isOpen, onClose, record }: { isOpen: boolean; onClose: () => void; record: PurchaseRecord | null }) {
  return (
          ...
          <div className="hide-scrollbar grow space-y-4 overflow-y-auto px-5 py-5">
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
              {[
                ...
                { label: "Grand Total", value: `₹${Number(record?.grandTotal ?? 0).toFixed(2)}`, bg: "bg-gradient-to-br from-primary-500 to-primary-700", Icon: BanknotesIcon },
                { label: "Basic Amount", value: `₹${Number(record?.totalBasic ?? 0).toFixed(2)}`, bg: "bg-gradient-to-br from-emerald-500 to-emerald-700", Icon: CurrencyRupeeIcon },
            ...
            <Card className="overflow-hidden">
              <div className="px-4 py-3 border-b border-gray-200 dark:border-dark-600 flex items-center gap-3 bg-gray-50 dark:bg-dark-800">
                ...
                <h3 className="text-sm font-bold text-gray-800 dark:text-dark-50">Line Items</h3>
...
export default function PurchaseRegisterPage() {
  // ✅ ADD: same role-gating as OutstandingReport + SalesEntryReport —
  ...
  const fetchRecords = useCallback(async (branchId: string = selectedBranchId) => {
    ...
    try {
      ...
      const res = await Get("pos/purchse-items/", params) as any;
      const body = res?.data ?? res;
      const rows: any[] = Array.isArray(body?.results) ? body.results
        : Array.isArray(body?.data) ? body.data
        : Array.isArray(body) ? body : [];
      setRecords(rows.map(mapApiPurchase));
    } catch {
      toasterrormsg("Failed to fetch purchase records.");
    } finally {
      setLoading(false);
    }
  }, [selectedBranchId]);
  ...
  const handleExport = () => {
    ...
    const a = document.createElement("a"); a.href = url; a.download = "purchase_register.csv"; a.click();
  ...
  const columns = useMemo<ColumnDef<PurchaseRecord>[]>(() => [
    ...
    },
    {
      ...
      cell: ({ getValue }: CellContext<PurchaseRecord, unknown>) => (
        <span className="block max-w-[120px] truncate text-gray-500 dark:text-dark-300">
          {String(getValue() ?? "") || "—"}
        </span>
      ),
    },
    ...
    <Page title="Purchase Register">
      <div className="transition-content w-full pb-8">
        ...
        <div className="px-(--margin-x) mt-2 grid grid-cols-2 gap-3 sm:grid-cols-4">
          {[
            { label: "Total Basic",  value: totals.totalBasic, bg: "bg-gradient-to-br from-primary-500 to-primary-700",  Icon: ShoppingBagIcon },
        ...
        <div className="px-(--margin-x) mt-4 max-w-sm">
          <Input value={globalFilter} onChange={e => setGlobalFilter(e.target.value)}
            ...
            placeholder="Search by Bill No, Party, Purchase Bill No…" />
        </div>
      ...
      {/* Items Detail Drawer */}
      <ItemsDrawer isOpen={!!selectedRecord} onClose={() => setSelectedRecord(null)} record={selectedRecord} />
    </Page>
  );
}