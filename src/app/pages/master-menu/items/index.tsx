...
import {
  ArrowPathIcon, EyeIcon, PencilSquareIcon,
...
function VariantsModal({
  itemId, itemName, branchFields, onClose,
}: {
  itemId: number | null;
  itemName: string;
  branchFields: string[];
  ...
  return (
    <Transition appear show={!!itemId} as={Fragment}>
          ...
          <div className="flex min-h-full items-center justify-center p-4">
            ...
            >
              <div className="flex items-center justify-between bg-primary px-5 py-4">
                <div>
                  <h3 className="text-base font-bold text-white">Item Variants</h3>
                  <p className="mt-0.5 text-xs text-white/70">{itemName}</p>
                </div>
                ...
                {loading ? (
                  ...
                  <table className="w-full text-sm">
                    <thead className="sticky top-0 bg-gray-100 dark:bg-dark-800">
                      <tr>
                        <th className="whitespace-nowrap px-4 py-2.5 text-left text-xs font-semibold uppercase text-gray-600 dark:text-dark-200">#</th>
                        {COLS.map(c => (
                          <th key={c} className="whitespace-nowrap px-4 py-2.5 text-left text-xs font-semibold uppercase text-gray-600 dark:text-dark-200">
                    ...
                    <tbody>
                      {variants.map((v, i) => (
                        <tr key={v.id ?? i}
                          ...
                          {COLS.map(c => (
                            <td key={c} className="px-4 py-2.5 text-gray-700 dark:text-dark-200">
                              {v[c] ?? "—"}
                            </td>
                          ))}
                        </tr>
